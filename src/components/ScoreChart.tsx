      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          {(["faithfulness", "correctness"] as MetricKey[]).map((m) => (
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-xs font-medium capitalize transition-colors ${
                metric === m ? "bg-ink text-surface" : "text-ink-2 hover:bg-wash"
              }`}
            >
              {m}
            </button>
        ...
        <div className="flex items-center gap-4 text-xs text-ink-2">
          <span className="inline-flex items-center gap-1.5">
            <span className="size-2.5 rounded-full bg-baseline-s" /> Baseline (live)
          </span>
          <span className="inline-flex items-center gap-1.5">
          ...
          {results.map((r, i) => {
            ...
            return (
              ...
              >
                ...
                <circle cx={x(b)} cy={cy} r={5} fill="var(--series-baseline)" stroke="var(--surface)" strokeWidth={2} />
                <circle cx={x(c)} cy={cy} r={5} fill="var(--series-candidate)" stroke="var(--surface)" strokeWidth={2} />
                {hovered && (
                  <g pointerEvents="none">
                    <rect
                      x={Math.min(Math.max(x(Math.min(b, c)) - 8, LABEL_W), W - 190)}
                      y={cy - 30}
                      width={182}
                      height={20}
                      rx={5}
                    ...
                    />
                    <text
                      x={Math.min(Math.max(x(Math.min(b, c)) - 8, LABEL_W), W - 190) + 8}
                      y={cy - 16}
                      fontSize={11}
                      fill="var(--surface)"
                      className="tabular-nums"
